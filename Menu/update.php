...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
    ...
    if (empty($name_err) && empty($featured_product_id_err) && empty($category_id_err)) {
        ...
        $sql = "UPDATE menus SET name=?, category_id=?,featured_product_id=?  WHERE id=?";
        $stmt = mysqli_prepare($link, $sql);
        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "siii", $param_name, $param_category_id, $param_featured_product_id, $param_id);
